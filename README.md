...
- Obsidian 风格面试复盘：源码编辑、自动拆块实时预览、完整预览、按投递和笔试/面试节点关联、搜索及 `.md` / PDF 导出
- 独立知识笔记：Markdown 编辑、实时预览、搜索及 `.md` / PDF 导出，代码块支持语言标签与常见编程语言语法高亮
- Dashboard、截止日期提醒，以及基于当日“进行中”流程节点生成的今日待办
- 实时搜索，以及状态、公司、地点和日期筛选
- 招聘流程节点支持分钟级时间、进行中/已完成状态、提前提醒，以及动画和声音提醒弹窗
- 看板仅允许向后拖动并要求填写节点时间；原节点自动完成，详情中的增量流程可撤销上一节点并同步清理其待办、提醒与复盘
- 投递趋势、笔试率、面试率和 Offer 率
- JSON 完整备份 / 恢复、CSV 导出
- 备份格式校验、覆盖前确认和重复记录标识修复
- Light / Dark / System 主题、Windows 开机自启，以及可新增、拖拽排序、配置复盘属性的流程编排
