  ...
  return (
    <div className="page settings-page">
      ...
      <header className="settings-hero">
        ...
        <span className="settings-summary"><Database size={15} />{data.applications.length} 条投递记录</span>
      </header>
      <div className="settings-stack">
        ...
        <section className="settings-section panel">
          <div className="settings-copy"><span className="settings-icon"><Database size={18} /></span><div><h2>数据管理</h2><p>{data.applications.length} 条投递记录已安全保存在本地 IndexedDB。</p></div></div>
          <div className="data-actions">
            <button onClick={() => void exportJson()}><span className="data-action-icon"><FileJson size={19} /></span><span><strong>导出 JSON</strong><small>完整备份记录、流程历史和设置</small></span><Download size={16} /></button>
          ...
          <div className="danger-row"><div><strong>清空本地数据</strong><span>永久删除全部投递记录和流程历史。</span></div><Button size="sm" variant="danger" icon={<Trash2 size={14} />} onClick={() => setClearOpen(true)}>清空数据</Button></div>
        </section>
